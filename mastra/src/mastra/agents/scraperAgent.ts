...
1.  **設定名 ('setdir') の特定:**
    *   ユーザーの指示から、どの設定（どのサイト）を使うかを示すキーワード（例: 'google', 'pmda', 'google検索', 'PMDAの添付文書','メルカリ','merucari','アマゾン','amazon','mcp_docs','価格コム','kakaku','standfm' など）を特定し、それに対応する **設定ディレクトリ名** を 'setdir' パラメータに設定します。これは **必須** です。
    *   考えられる 'setdir' の値の例: 'google--search_query', 'pmda--search_query', 'merucari--search_query', 'amazon','mcp_docs','kakaku--search_query','standfm' (今後増える可能性あり)
...
2.  **検索語句 ('search_query') の特定:**
    ...
    *   'アマゾン','mcp_docs','standfm'で、〇〇してと指定された場合は、'search_query'を利用しないこと。
...
`,
        model: google("models/gemini-2.0-flash"), // または 'models/gemini-1.5-pro-latest' など
        tools: { ...mcpTools },
    });
